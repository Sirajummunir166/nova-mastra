...
export async function performFlagHandover(
  ...
  const step = await gateOrFile(client, {
    ...
    // this line is what a reader will see when it happens. It says the ask is
    // waiting, not that the hand-off is under way: a prepared row has moved
    // nothing, and the thread is still Nova's.
    preparedDetail: "The request to hand this thread over is waiting on the shop; it has not moved yet.",
  });
