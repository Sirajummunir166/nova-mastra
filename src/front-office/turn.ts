...
export function buildActionLine(
  ...
  switch (action) {
    ...
    case "ORDER_PENDING_APPROVAL":
      ...
      bits.push(
        "confirm warmly that the order is being placed — the shop double-checks every chat order before dispatch; this is the normal flow, not a problem, so no apology; do NOT promise when they will confirm, and do NOT invent an order number or a final total",
      );
      break;
    case "ESCALATE":
      ...
      if (opts.handoverOutcome === "blocked") {
        ...
        bits.push(
          "the hand-off did not go through, so do NOT say anyone is taking over, will reply, or will call back; answer the customer yourself from what the state above supports, say plainly if it is something you cannot settle here, and never mention the shop's internal rules",
        );
      } else {
